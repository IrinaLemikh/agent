...
class Glossary:
    ...
    # =========================================================================
    def to_prompt_text(self, section: str = "problem") -> str:
        """
        Словарь плоским текстом, готовым для вставки в промпт LLM — но
        только записи нужного раздела (поле "section" в glossary.json).
        Записи без явного "section" считаются разделом "problem" (это все
        исходные ~30 терминов техподдержки — им не нужно было ничего менять
        при введении разделов). "client_address" — отдельный маленький блок
        для батча нормализации клиент+адрес (см. обсуждение в чате: модели
        для этого вызова не нужен технический жаргон про ФН/УТМ/ЧЗ и т.п.,
        только контекст про суб-бренды вида "Пивко Проф").
        ...
        """
        relevant = {
            term: entry for term, entry in self.entries.items()
        ...
        lines = []
        for term in sorted(relevant.keys()):
            entry = relevant[term]
    ...
    # =========================================================================
    # НОВОЕ: обратный словарь alias -> канонический термин, для батча
    # клиент+адрес (core/llm/client.py: normalize_client_address_batch).
    # LLM видит контекст через to_prompt_text(section=...) и обычно сама
    # извлекает уже развёрнутую форму, но полагаться на это на 100% нельзя
    # (см. обсуждение в чате про несогласованность LLM между батчами) — этот
    # метод даёт Python-гарантию канонического вида как страховку постфактум.
    # =========================================================================
